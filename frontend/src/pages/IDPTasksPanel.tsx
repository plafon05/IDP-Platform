      ...
      {showForm && editable && (
        <form className="task-form" onSubmit={submit}>
          ...
          <div className="form-grid">
            ...
            <label className="form-field date-field"><span>Срок</span><input type="date" min={plan.start_date} max={plan.end_date} value={form.due_date ?? ''} onClick={(event) => event.currentTarget.showPicker?.()} onChange={(e) => setForm({ ...form, due_date: e.target.value })} /><small>Период ИПР: {formatDate(plan.start_date)} - {formatDate(plan.end_date)}</small></label>
