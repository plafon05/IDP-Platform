...
const emptyForm: TemplatePayload = { title: '', description: '', goals: '', target_role: '', is_active: true, tasks: [{ ...emptyTask }], competencies: [] };
...
export function TemplatesPage() {
  ...
  function reset() { setEditingID(null); setForm({ ...emptyForm, tasks: [{ ...emptyTask }], competencies: [] }); }
  function setTask(index: number, value: TemplateTask) { setForm((current) => ({ ...current, tasks: current.tasks.map((task, i) => i === index ? value : task) })); }
  ...
  return <div className="templates-page">
    ...
    <section className="templates-layout">
      ...
      <form className="panel template-form" onSubmit={submit}><div className="panel-header"><div><h2>{editingID ? 'Редактирование шаблона' : 'Новый шаблон'}</h2><p>Сроки задач задаются в днях от начала ИПР</p></div><Plus size={20} /></div>
        ...
        <div className="template-tasks"><div className="task-panel-heading"><strong>Задачи</strong><button className="secondary-button compact" type="button" onClick={() => setForm((current) => ({ ...current, tasks: [...current.tasks, { ...emptyTask }] }))}><Plus size={15} />Добавить</button></div>{form.tasks.map((task, index) => <div className="template-task-editor" key={index}><label className="form-field"><span>Название</span><input required maxLength={200} value={task.title} onChange={(event) => setTask(index, { ...task, title: event.target.value })} /></label><label className="form-field"><span>Описание</span><input value={task.description ?? ''} onChange={(event) => setTask(index, { ...task, description: event.target.value })} /></label><div className="form-grid"><label className="form-field"><span>Категория</span><select value={task.category_id ?? ''} onChange={(event) => setTask(index, { ...task, category_id: event.target.value || undefined })}><option value="">Без категории</option>{categories.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="form-field"><span>Приоритет</span><select value={task.priority} onChange={(event) => setTask(index, { ...task, priority: event.target.value as TemplateTask['priority'] })}><option value="low">Низкий</option><option value="medium">Средний</option><option value="high">Высокий</option></select></label><label className="form-field"><span>День срока</span><input type="number" min={0} max={3650} value={task.due_offset_days ?? ''} onChange={(event) => setTask(index, { ...task, due_offset_days: event.target.value === '' ? undefined : Number(event.target.value) })} /></label></div>{form.tasks.length > 1 && <button className="icon-button danger" type="button" aria-label="Удалить задачу" onClick={() => setForm((current) => ({ ...current, tasks: current.tasks.filter((_, i) => i !== index) }))}><Trash2 size={16} /></button>}</div>)}</div>
